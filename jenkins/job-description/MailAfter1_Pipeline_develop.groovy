...
def job_config = [
        job: [
                name: "MailAfter1_Pipeline_develop"
        ],
        git: [
                branch: "develop"
        ]
...
def customLib = library identifier: 'MailAfter1_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kkanto/MailAfter1_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = lib.de.allianz.MailAfter1.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                    jslGit.checkout( config, "JEQP", "MailAfter1_Pipeline_develop", "develop")
                }
            }    
        }
        stage('Build') {
